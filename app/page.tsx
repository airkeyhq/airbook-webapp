    ...
    <div className="min-h-screen bg-[#FAFAFC] dark:bg-[#0C0D12] text-[#0F172A] dark:text-[#F8FAFC] flex flex-col justify-between selection:bg-pink-500 selection:text-white relative overflow-x-hidden font-sans">
      ...
      <header className="sticky top-0 z-50 w-full backdrop-blur-2xl bg-white/80 dark:bg-[#0C0D12]/80 border-b border-slate-200 dark:border-white/10 transition-all">
        <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            {/* Circle Flag Language Switcher */}
            <div className="relative z-50">
              <button
                onClick={() => setIsLangMenuOpen(!isLangMenuOpen)}
                className="h-9 flex items-center gap-1.5 px-3 rounded-full bg-slate-100 dark:bg-white/10 hover:bg-slate-200 dark:hover:bg-white/20 transition-colors text-xs font-bold text-slate-700 dark:text-slate-200"
              >
                <img
                ...
                />
                <span className="uppercase text-[11px] font-extrabold">{currentLangObj.id}</span>
                <ChevronDown24Filled className="w-3 h-3 text-slate-400" />
              </button>

              {isLangMenuOpen && (
                <div className="absolute right-0 mt-2 w-36 py-1 bg-white dark:bg-[#1A1D26] rounded-2xl shadow-xl border border-slate-200 dark:border-white/10 z-50 overflow-hidden">
      ...
      <section id="features" className="max-w-5xl mx-auto px-6 py-20 border-t border-slate-200 dark:border-white/10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          >
            ...
            <h3 className="text-lg font-bold text-[#0F172A] dark:text-[#F8FAFC]">{t('feat1Title')}</h3>
          ...
          >
            ...
            <h3 className="text-lg font-bold text-[#0F172A] dark:text-[#F8FAFC]">{t('feat2Title')}</h3>
            <p className="text-xs text-slate-600 dark:text-slate-300 leading-relaxed">
              {t('feat2Desc')}
            </p>
          </motion.div>
          ...
          >
            ...
            <h3 className="text-lg font-bold text-[#0F172A] dark:text-[#F8FAFC]">{t('feat3Title')}</h3>
            <p className="text-xs text-slate-600 dark:text-slate-300 leading-relaxed">
              {t('feat3Desc')}
            </p>
          </motion.div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          >
            <div className="flex justify-between items-start">
              <div>
                <h3 className="text-lg font-extrabold text-[#0F172A] dark:text-[#F8FAFC]">{t('soloPlanTitle')}</h3>
                <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">{t('soloPlanDesc')}</p>
              </div>
      ...
      <footer className="w-full max-w-6xl mx-auto px-6 py-8 border-t border-slate-200 dark:border-white/10 flex flex-col sm:flex-row items-center justify-between gap-4 text-xs text-slate-500 dark:text-slate-400">
        ...
        <div className="flex items-center gap-6 font-medium">
          <Link href="/dashboard" className="hover:underline">{t('operatorApp')}</Link>
          <Link href="/login" className="hover:underline">{t('signIn')}</Link>
          <Link href="/book/eduardos-lounge" className="hover:underline">{t('seeLiveDemo')}</Link>
        </div>
      </footer>
    </div>
  );

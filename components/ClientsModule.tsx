...
import { Add24Filled, Dismiss24Filled, Person24Regular, Comment24Regular, Mail24Regular, Phone24Regular, ChevronRight24Regular } from '@fluentui/react-icons';
      ...
      {!loading && (
        <>
          {clients.length === 0 ? (
            ...
            <div className="divide-y divide-black/5 dark:divide-white/5 border border-black/5 dark:border-white/10 rounded-3xl overflow-hidden bg-white/70 dark:bg-gray-900/70 glass-panel shadow-sm">
              {clients.map((cli) => {
                  ...
                  >
                    <div className="flex items-start gap-3.5 min-w-0">
                      <img
                        src={avatarSrc}
                        alt={cli.name}
                        className="w-10 h-10 rounded-2xl object-cover shadow-sm group-hover:scale-105 transition-transform duration-100 ease-out flex-shrink-0"
                      />
                      <div className="min-w-0">
                        <h3 className="text-sm font-bold text-[var(--text-primary)] group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors flex items-center gap-2">
                          ...
                          <span className="px-2 py-0.5 rounded-full bg-blue-500/10 text-blue-600 dark:text-blue-400 text-[10px] font-semibold flex-shrink-0">
                            {cli.totalVisits} {t('visits')}
                          </span>
                        </h3>
                        <p className="text-xs text-[var(--text-secondary)] mt-0.5 flex flex-wrap items-center gap-x-3 gap-y-1">
                          {cli.email && <span className="flex items-center gap-1 truncate"><Mail24Regular className="w-3 h-3 text-[var(--text-muted)]" />{cli.email}</span>}
                          {cli.phone && <span className="flex items-center gap-1 truncate"><Phone24Regular className="w-3 h-3 text-[var(--text-muted)]" />{cli.phone}</span>}
                        </p>

                        {/* Preference Notes */}
                        {cli.notes && (
                          <div className="mt-2 flex items-center gap-1.5 text-xs text-[var(--text-secondary)] bg-black/5 dark:bg-white/5 px-2.5 py-1 rounded-xl w-fit max-w-full">
                    ...
                    <div className="flex items-center justify-between sm:justify-end gap-3 flex-shrink-0">
                      ...
                      <ChevronRight24Regular className="w-4 h-4 text-[var(--text-muted)] group-hover:text-blue-500 group-hover:translate-x-0.5 transition-all" />
                    </div>
                  </div>
                );
              })}
